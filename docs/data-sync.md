...
which validates the resulting session state as a whole (`assertContainedWalkingSession`)
before building every correction, every undo and every delete, and is the
only path the application uses to construct these mutations. See [PAD
walking: Editing, undo, and delete](pad-walking.md#editing-undo-and-delete)
for the correction validation rules, the undo contract (a forward
compensating mutation, derived from an explicit transition stamp on the
session record rather than an undo stack), and the delete-with-renumber
mutation shape.

**Correction (issue #22, finding 2):** at the time this paragraph was first
written, `assertContainedWalkingSession` was called only from the three
`correct*` builders -- undo and delete built their mutations unchecked, which
made the claim above false for two of the three kinds of mutation this
paragraph describes, and was the concrete opening finding 1's stamp fix (see
[PAD walking: Undo](pad-walking.md#undo)) closed: a wrong undo target could
otherwise have been committed rather than refused. Both now call it too
(`frontend/src/pad/actions.ts`, `undoLastWalkingTransitionAction` and
`deleteWalkingBoutAction`), against the *hypothetical resulting* bouts/pauses/
rests before any change is pushed onto the `LocalAction`, so the sentence
above is accurate as written.
